...
가장 먼저 생각난 방법은 `AmazonS3.putObject()`를 `mocking`하기 위해  `@MockBean` `@SpyBean`를 사용하는 방법 이었습니다.  

하지만 `@MockBean` `@SpyBean` 을 사용하게 되면 Spring Context를 실행해야 한다는 단점 때문에 이 방법은 사용하지 않기로 하였습니다. 

다른 방법으로는 제어할 수 없는 코드의 위치를 최대한 바깥(Controller)으로 밀어내는 방법도 있지만, 모든 계층에서 값을 파라미터로 넘겨줘야 하기 때문에 이 방법 또한 사용하지 않기로 하였습니다.

이러한 이유로 테스트 하기 어려운 코드들을 새로운 컴포넌트로 정의하여 **의존성으로 주입**하는 방법을 선택하였습니다.
...
하지만 기존에 테스트 코드에서 우리가 제어할 수 없던 동작들을 **우리의 마음대로 제어할 수 있게** 되었습니다.
...
또, 다음 포스팅에서 더욱 자세하게 다룰 예정이지만, 의존성 주입을 잘 활용하는 것으로 `@MockBean` 없는 테스트를 더욱 수월하게 작성할 수 있게 됩니다.
